...
class TestGetArticleRawHtmlDeletedArticle:
    """Tests for deleted article handling in get_article_raw_html().

    Issue #209: get_article_raw_html should also check for deleted status
    to prevent operations on deleted articles (e.g., image operations).
    """
    ...
    @pytest.mark.asyncio
    async def test_get_article_raw_html_raises_error_for_deleted_article(self, mock_session: Session) -> None:
        """get_article_raw_html() should raise NoteAPIError for deleted articles.
