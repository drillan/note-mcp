...
使用方法: $source-command-merge-pr <PR番号> [--merge|--rebase]
例: $source-command-merge-pr 45
例: $source-command-merge-pr 45 --merge（マージコミットを作成）
例: $source-command-merge-pr 45 --rebase（リベースマージ）
```
...
- worktreeの自動削除は`.codex/git-conventions.md`の命名規則に基づいて検出する
