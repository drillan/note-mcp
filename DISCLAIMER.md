# 免責事項 / Disclaimer

## 日本語

### 非公式ツール

このプロジェクト（note-mcp）は、note.com（note株式会社）とは一切関係のない非公式のサードパーティツールです。

### 自己責任

**このツールの使用は完全に自己責任となります。**

- note.comの利用規約を確認のうえご利用ください
- note.comのAPI仕様が予告なく変更され、ツールが動作しなくなる可能性があります
- アカウントの停止や制限を受ける可能性があります
- データの損失や破損が発生する可能性があります

### 無保証

**このソフトウェアは「現状のまま」（AS IS）で提供され、いかなる明示的または暗黙的な保証もありません。**

以下を含むがこれに限定されない、いかなる損害についても、作者および貢献者は責任を負いません：

- 直接的、間接的、偶発的、特別、懲罰的、または結果的な損害
- 利益、営業、データ、使用の損失
- 事業の中断

### 推奨事項

このツールを使用する前に：

1. note.comの利用規約を確認してください
2. 重要なコンテンツは別途バックアップしてください
3. テスト用アカウントでの動作確認を推奨します
4. 過度なリクエストを送信しないでください（レート制限: 10リクエスト/分を目安）

### 適切な使用について

note-mcpは個人の記事管理を効率化するツールです。
次のような**過度な使用や悪用**は避けてください。

| 避けるべき使用方法 | 理由 |
|------------------|------|
| 短時間での大量リクエスト | サーバーへの負荷（noteクリエイター規約・禁止事項） |
| 自動化による連続投稿 | スパム行為とみなされる可能性（noteクリエイター規約・ご利用の停止） |
| 他者のコンテンツの無断複製 | 著作権侵害（noteクリエイター規約・禁止事項） |

### 参考リンク

- [note ご利用規約](https://terms.help-note.com/hc/ja/articles/44943817565465)

---

## English

### Unofficial Tool

This project (note-mcp) is an unofficial third-party tool that has no affiliation with note.com (note Inc.).

### Use at Your Own Risk

**The use of this tool is entirely at your own risk.**

- Please review note.com's Terms of Service before use
- note.com's API specifications may change without notice, causing the tool to stop working
- Your account may be suspended or restricted
- Data loss or corruption may occur

### No Warranty

**This software is provided "AS IS" without warranty of any kind, express or implied.**

The authors and contributors shall not be liable for any damages whatsoever, including but not limited to:

- Direct, indirect, incidental, special, punitive, or consequential damages
- Loss of profits, business, data, or use
- Business interruption

### Recommendations

Before using this tool:

1. Review note.com's Terms of Service
2. Back up important content separately
3. Test with a test account first
4. Avoid sending excessive requests (rate limit: ~10 requests/minute)

### Appropriate Use

note-mcp is a tool designed to streamline personal article management.
Please avoid the following **excessive use or misuse**:

| Usage to Avoid | Reason |
|----------------|--------|
| Bulk requests in short periods | Server load (note Creator Terms: Prohibited Actions) |
| Automated continuous posting | May be considered spam (note Creator Terms: Service Suspension) |
| Unauthorized copying of others' content | Copyright infringement (note Creator Terms: Prohibited Actions) |

### Reference Links

- [note Terms of Service](https://terms.help-note.com/hc/ja/articles/44943817565465) (Japanese)

---

## ライセンス / License

MIT License - 詳細は[LICENSE](LICENSE)ファイルを参照してください。
