...
| `note_publish_article` | 記事を公開（有料・限定・マガジン設定対応） |
| `note_get_separator_candidates` | 有料エリア境界に使う本文ブロックUUID候補を取得 |
| `note_set_paid_settings` | 下書きに価格・有料エリア境界を保存（公開しない） |
| `note_list_my_magazines` | 自分が所有するマガジン一覧を取得 |
| `note_list_circle_plans` | 紐づけ可能なメンバーシッププラン一覧を取得 |
