...
async def extract_math_elements(page: Page) -> dict[str, Any]:
    ...
    () => {
        ...
        return {
            ...
            // Check for unconverted math patterns
            raw_patterns: {
                dollar_single: (document.body.textContent?.match(/\$[^$]+\$/g) || []).length,
...
async def main() -> int:
    ...
    try:
        ...
        try:
            ...
            unconverted_count = raw_patterns.get("dollar_single", 0)
            ...
            if has_rendered_math and unconverted_count == 0:
                ...
                success = False
                message = f"Partial rendering: {nwc_formula_count} converted, {unconverted_count} unconverted $...$ patterns remain"
            else:
                success = False
