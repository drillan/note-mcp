...
1. **Markdown入力**: `[TOC]` 形式で目次を記述
2. **API経由で処理**: note-mcpが `<table-of-contents>` カスタム要素に直接変換
3. **HTMLレンダリング**: note.comのフロントエンドが目次を自動生成・表示

> **📝 2026-01-12更新 (Issue #117)**: 目次機能がAPI化されました。ブラウザ自動化を経由せず、直接APIで記事を作成できます。

### API経由の処理

`[TOC]` マーカーは `<table-of-contents>` カスタム要素に変換され、API経由で保存されます：

```html
<table-of-contents name="uuid" id="uuid"></table-of-contents>
```

note.comのフロントエンドがこのカスタム要素を検出し、記事内の見出しから目次を自動生成します。
