...
note-mcpが提供する13のMCPツールのリファレンスです。
...
ブラウザ自動化を使用してnote.comエディタにMarkdownを入力します。作成後、ブラウザでプレビューを表示します。
...
| `article_key` | str | はい | 画像を挿入する記事のキー（例: n1234567890ab） |
| `caption` | str | いいえ | 画像のキャプション |

**動作**

note.comのAPIでは画像のHTML埋め込みが正しく保存されないため、ブラウザ経由でエディタの「画像を追加」機能を使用します。

**戻り値**

```
画像を挿入しました。記事キー: n1234567890ab、キャプション: 図1. システム構成図
```
