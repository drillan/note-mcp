...
公開前に、下書き本文のブロックUUIDを取得して有料エリア境界を指定します。

```
記事ID n1234567890ab の有料エリア境界候補を取得してください
記事ID n1234567890ab に価格500円、有料エリア境界UUID <uuid> を保存してください
記事ID n1234567890ab を価格500円、有料エリア境界UUID <uuid> で公開してください
```

メンバーシップ限定にする場合は、先に`note_list_circle_plans`でプランキーを確認してから
`circle_plan_keys`を指定します。マガジンに追加する場合は`note_list_my_magazines`で
`magazine_keys`を確認します。
