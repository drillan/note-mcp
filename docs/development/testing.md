...
MCPツールテストは、note-mcpが提供する21個のMCPツールの動作を検証するE2Eテストです。
...
| | `note_get_separator_candidates` | 有料エリア境界候補を取得 |
| | `note_set_paid_settings` | 下書きに有料設定を保存 |
| | `note_list_my_magazines` | マガジン一覧を取得 |
| | `note_list_circle_plans` | メンバーシッププラン一覧を取得 |
