...
- Q: テスト対象に追加すべき編集機能の範囲は？ → A: 全記法（TOC、引用、リスト、番号付きリスト、リンク、太字、斜体、水平線）
- Q: 目次（TOC）の検証方法は？ → A: プレビューページでTOC要素（TableOfContentsクラス）を検証
- Q: 目次（TOC）のエディタへの入力方法は？ → A: `[TOC]`プレースホルダー入力後、MCPツール経由でTOC挿入
...
- Q: 太字・斜体のProseMirrorトリガーパターンは？ → A: `**text**` / `*text*` + スペースで変換
...
- **FR-011**: テストは以下のすべてのMarkdown記法をサポートしなければならない（全てP1優先度）：
  ...
  - コードブロック
  - テキスト配置
  - 目次（TOC）- `[TOC]`プレースホルダー＋MCPツール挿入、プレビューページのTableOfContentsクラスで検証
  - 引用（blockquote） - 行頭で `> ` を入力してブロックモードに変換
  - リスト（箇条書き） - 行頭で `- ` を入力してブロックモードに変換
  - 番号付きリスト - 行頭で `1. ` を入力してブロックモードに変換
  - リンク - `[text](url)` + スペースで変換
  - 太字（bold） - `**text**` + スペースで変換
  - 斜体（italic） - `*text*` + スペースで変換
  - 水平線 - `---` + Enterで変換

  **注**: 各記法の期待HTMLマッピングは別仕様（Markdown-HTML変換仕様）で定義する
...
- **SC-005**: FR-011で定義された12種類すべてのMarkdown記法が検証可能
